	...
	if(reportLen>=28 && (offset + reportLen)<=mem->GetSize())
	{
		...
		if((x[offset] & 0x03)==2 && x[offset+1]==0xc8)
		{
			if((28 + (rCount * 24)) <= reportLen)
			{
				...
				m_time = Memory::toLong(mem,offset + 8);
				m_rtpTime = Memory::toInt(mem,offset + 16);
				m_senderPacketCount = Memory::toInt(mem,offset + 20);
				m_senderOctetCount = Memory::toInt(mem,offset + 24);
...
bool RTCPPacketSR::packet(NetArraySPtr mem)
{
	...
	Memory::fromLong(x,8,static_cast<tuint64>(m_time));
	Memory::fromInt(x,16,static_cast<tuint32>(m_rtpTime));
	Memory::fromInt(x,20,m_senderPacketCount);
	Memory::fromInt(x,24,m_senderOctetCount);
