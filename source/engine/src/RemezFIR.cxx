...
void RemezFIR::createDenseGrid(int r, int numtaps, int numband, const double bands[],
...
void RemezFIR::calcError(int r, double ad[], double x[], double y[],
...
int RemezFIR::search(int r, int Ext[], int gridsize, double E[])
{
...
void RemezFIR::freqSample(int N, double A[], double h[], int symm)
{
   int n, k;
   double x, val, M;
...
int RemezFIR::isDone(int r, int Ext[], double E[])
{
   int i;
...
void RemezFIR::remez(double h[], int *numtaps, int *numband, const double bands[], 
      const double des[], const double weight[], int *type, int *griddensity)
{
	...
	for (iter=0; iter<MAXITERATIONS; iter++)
	{
		...
		if (err)
		{
            common::Log::g_Log.print("Remez Exchange error, %d, %d", err, gridsize);
		} 
		if (isDone(r, Ext, E))
			break;
	}
