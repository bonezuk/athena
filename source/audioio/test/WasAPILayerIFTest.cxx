...
TEST(WasAPIDeviceLayer,getNumberOfBitsFromIndex)
{
	...
	EXPECT_EQ(8,device.testGetNumberOfBitsFromIndex(0));
	EXPECT_EQ(16,device.testGetNumberOfBitsFromIndex(1));
	EXPECT_EQ(24,device.testGetNumberOfBitsFromIndex(2));
	EXPECT_EQ(32,device.testGetNumberOfBitsFromIndex(3));
	EXPECT_EQ(-1,device.testGetNumberOfBitsFromIndex(4));
}
...
TEST(WasAPIDeviceLayer,findClosestFormatTypeSupportedFor16Bits44100HzAnd2ChannelsWhen16Bits44100HzAnd2ChannelsSupported)
{
	...
	WAVEFORMATEX formatActual;
	device.testDefaultWaveFormat(formatActual);
	device.testSetWaveFormat(2,16,44100,formatActual);	
	
	WAVEFORMATEX formatSupported;
	device.testDefaultWaveFormat(formatSupported);
	device.testSetWaveFormat(2,16,44100,formatSupported);

	WasAPIDeviceLayerFindDefaultFormatTypeInvoker invoker;
	invoker.append(&formatSupported);

	EXPECT_CALL(mockAudio,IsFormatSupported(AUDCLNT_SHAREMODE_EXCLUSIVE,A<const WAVEFORMATEX *>(),0))
		.WillRepeatedly(Invoke(&invoker,&WasAPIDeviceLayerFindDefaultFormatTypeInvoker::IsFormatSupported));

	WAVEFORMATEX *pFormat = device.findClosestFormatType(&formatActual);
	
	ASSERT_TRUE(pFormat!=0);
	
	EXPECT_EQ(WAVE_FORMAT_PCM,pFormat->wFormatTag);
	EXPECT_EQ(2,pFormat->nChannels);
	EXPECT_EQ(44100,pFormat->nSamplesPerSec);
	EXPECT_EQ(44100 * 2 * 2,pFormat->nAvgBytesPerSec);
	EXPECT_EQ(4,pFormat->nBlockAlign);
	EXPECT_EQ(16,pFormat->wBitsPerSample);
	EXPECT_EQ(0,pFormat->cbSize);
...
TEST(WasAPIDeviceLayer,findClosestFormatTypeSupportedFor16Bits44100HzAnd2ChannelsWhen24Bits44100HzAnd2ChannelsSupported)
{
	...
	WAVEFORMATEX formatSupported;
	device.testDefaultWaveFormat(formatSupported);
	device.testSetWaveFormat(2,24,44100,formatSupported);
	...
	EXPECT_EQ(2,pFormat->nChannels);
	EXPECT_EQ(44100,pFormat->nSamplesPerSec);
	EXPECT_EQ(44100 * 3 * 2,pFormat->nAvgBytesPerSec);
	EXPECT_EQ(6,pFormat->nBlockAlign);
	EXPECT_EQ(24,pFormat->wBitsPerSample);
	EXPECT_EQ(0,pFormat->cbSize);
...
TEST(WasAPIDeviceLayer,findClosestFormatTypeSupportedFor16Bits48000HzAnd2ChannelsWhen16Bits96000HzAnd8ChannelsSupported)
{
	...
	WAVEFORMATEX formatActual;
	device.testDefaultWaveFormat(formatActual);
	device.testSetWaveFormat(2,16,48000,formatActual);	
	
	WAVEFORMATEX formatSupported;
	device.testDefaultWaveFormat(formatSupported);
	device.testSetWaveFormat(8,16,96000,formatSupported);
	...
	EXPECT_EQ(WAVE_FORMAT_PCM,pFormat->wFormatTag);
	EXPECT_EQ(8,pFormat->nChannels);
	EXPECT_EQ(96000,pFormat->nSamplesPerSec);
	EXPECT_EQ(96000 * 2 * 8,pFormat->nAvgBytesPerSec);
	EXPECT_EQ(16,pFormat->nBlockAlign);
	EXPECT_EQ(16,pFormat->wBitsPerSample);
	EXPECT_EQ(0,pFormat->cbSize);
...
TEST(WasAPIDeviceLayer,findClosestFormatTypeSupportedFor16Bits48000HzAnd2ChannelsWhen32BitFloat88200HzAnd6ChannelsSupported)
{
	...
	WAVEFORMATEX formatSupported;
	device.testDefaultWaveFormat(formatSupported);
	formatSupported.wFormatTag = WasAPIDeviceLayer::e_formatFloat;
	device.testSetWaveFormat(6,32,88200,formatSupported);
	...
	EXPECT_EQ(WasAPIDeviceLayer::e_formatFloat,pFormat->wFormatTag);
	EXPECT_EQ(6,pFormat->nChannels);
	EXPECT_EQ(88200,pFormat->nSamplesPerSec);
	EXPECT_EQ(88200 * 4 * 6,pFormat->nAvgBytesPerSec);
	EXPECT_EQ(24,pFormat->nBlockAlign);
	EXPECT_EQ(32,pFormat->wBitsPerSample);
	EXPECT_EQ(0,pFormat->cbSize);
...
TEST(WasAPIDeviceLayer,findClosestFormatTypeSupportedFor16Bits44100HzAnd2ChannelsWhen16Bits22050HzAnd2ChannelsSupported)
{
	...
	WAVEFORMATEX formatSupported;
	device.testDefaultWaveFormat(formatSupported);
	device.testSetWaveFormat(2,16,22050,formatSupported);
	...
	EXPECT_EQ(WAVE_FORMAT_PCM,pFormat->wFormatTag);
	EXPECT_EQ(2,pFormat->nChannels);
	EXPECT_EQ(22050,pFormat->nSamplesPerSec);
	EXPECT_EQ(22050 * 2 * 2,pFormat->nAvgBytesPerSec);
	EXPECT_EQ(4,pFormat->nBlockAlign);
	EXPECT_EQ(16,pFormat->wBitsPerSample);
	EXPECT_EQ(0,pFormat->cbSize);
...
TEST(WasAPIDeviceLayer,findClosestFormatTypeSupportedFor16Bits48000HzAnd2ChannelsWhen24Bits32000HzAnd8ChannelsSupported)
{
	...
	WAVEFORMATEX formatSupported;
	device.testDefaultWaveFormat(formatSupported);
	device.testSetWaveFormat(8,24,32000,formatSupported);
	...
	EXPECT_EQ(WAVE_FORMAT_PCM,pFormat->wFormatTag);
	EXPECT_EQ(8,pFormat->nChannels);
	EXPECT_EQ(32000,pFormat->nSamplesPerSec);
	EXPECT_EQ(32000 * 3 * 8,pFormat->nAvgBytesPerSec);
	EXPECT_EQ(24,pFormat->nBlockAlign);
	EXPECT_EQ(24,pFormat->wBitsPerSample);
	EXPECT_EQ(0,pFormat->cbSize);
...
TEST(WasAPIDeviceLayer,findClosestFormatTypeSupportedFor16Bits48000HzAnd4ChannelsWhen16Bits96000HzAnd2ChannelsSupported)
{
	...
	WAVEFORMATEX formatActual;
	device.testDefaultWaveFormat(formatActual);
	device.testSetWaveFormat(4,16,48000,formatActual);	
	
	WAVEFORMATEX formatSupported;
	device.testDefaultWaveFormat(formatSupported);
	device.testSetWaveFormat(2,16,96000,formatSupported);
	...
	EXPECT_EQ(2,pFormat->nChannels);
	EXPECT_EQ(96000,pFormat->nSamplesPerSec);
	EXPECT_EQ(96000 * 2 * 2,pFormat->nAvgBytesPerSec);
	EXPECT_EQ(4,pFormat->nBlockAlign);
	EXPECT_EQ(16,pFormat->wBitsPerSample);
	EXPECT_EQ(0,pFormat->cbSize);
...
TEST(WasAPIDeviceLayer,findClosestFormatTypeSupportedFor16Bits48000HzAnd6ChannelsWhen32BitFloat88200HzAnd1ChannelsSupported)
{
	...
	WAVEFORMATEX formatActual;
	device.testDefaultWaveFormat(formatActual);
	device.testSetWaveFormat(6,16,48000,formatActual);	
	
	WAVEFORMATEX formatSupported;
	device.testDefaultWaveFormat(formatSupported);
	formatSupported.wFormatTag = WasAPIDeviceLayer::e_formatFloat;
	device.testSetWaveFormat(1,32,88200,formatSupported);
	...
	EXPECT_EQ(WasAPIDeviceLayer::e_formatFloat,pFormat->wFormatTag);
	EXPECT_EQ(1,pFormat->nChannels);
	EXPECT_EQ(88200,pFormat->nSamplesPerSec);
	EXPECT_EQ(88200 * 1 * 4,pFormat->nAvgBytesPerSec);
	EXPECT_EQ(4,pFormat->nBlockAlign);
	EXPECT_EQ(32,pFormat->wBitsPerSample);
	EXPECT_EQ(0,pFormat->cbSize);
...
TEST(WasAPIDeviceLayer,findClosestFormatTypeSupportedFor16Bits48000HzAnd8ChannelsWhen24Bits32000HzAnd4ChannelsSupported)
{
	...
	WAVEFORMATEX formatActual;
	device.testDefaultWaveFormat(formatActual);
	device.testSetWaveFormat(8,16,48000,formatActual);	
	
	WAVEFORMATEX formatSupported;
	device.testDefaultWaveFormat(formatSupported);
	device.testSetWaveFormat(4,24,32000,formatSupported);
	...
	EXPECT_EQ(WAVE_FORMAT_PCM,pFormat->wFormatTag);
	EXPECT_EQ(4,pFormat->nChannels);
	EXPECT_EQ(32000,pFormat->nSamplesPerSec);
	EXPECT_EQ(32000 * 4 * 3,pFormat->nAvgBytesPerSec);
	EXPECT_EQ(12,pFormat->nBlockAlign);
	EXPECT_EQ(24,pFormat->wBitsPerSample);
	EXPECT_EQ(0,pFormat->cbSize);
...
TEST(WasAPIDeviceLayer,findClosestFormatTypeSupportedFor24Bits44100HzAnd2ChannelsWhen8Bits44100HzAnd2ChannelsSupported)
{
	...
	WAVEFORMATEX formatActual;
	device.testDefaultWaveFormat(formatActual);
	device.testSetWaveFormat(2,24,44100,formatActual);	
	
	WAVEFORMATEX formatSupported;
	device.testDefaultWaveFormat(formatSupported);
	device.testSetWaveFormat(2,8,44100,formatSupported);
	...
	EXPECT_EQ(2,pFormat->nChannels);
	EXPECT_EQ(44100,pFormat->nSamplesPerSec);
	EXPECT_EQ(44100 * 2 * 1,pFormat->nAvgBytesPerSec);
	EXPECT_EQ(2,pFormat->nBlockAlign);
	EXPECT_EQ(8,pFormat->wBitsPerSample);
	EXPECT_EQ(0,pFormat->cbSize);
...
TEST(WasAPIDeviceLayer,findClosestFormatTypeSupportedFor32BitFloat48000HzAnd6ChannelsWhen16Bits88200HzAnd2ChannelsSupported)
{
	...
	WAVEFORMATEX formatActual;
	device.testDefaultWaveFormat(formatActual);
	formatActual.wFormatTag = WasAPIDeviceLayer::e_formatFloat;
	device.testSetWaveFormat(6,32,48000,formatActual);	
	
	WAVEFORMATEX formatSupported;
	device.testDefaultWaveFormat(formatSupported);
	device.testSetWaveFormat(2,16,88200,formatSupported);
	...
	EXPECT_EQ(2,pFormat->nChannels);
	EXPECT_EQ(88200,pFormat->nSamplesPerSec);
	EXPECT_EQ(88200 * 2 * 2,pFormat->nAvgBytesPerSec);
	EXPECT_EQ(4,pFormat->nBlockAlign);
	EXPECT_EQ(16,pFormat->wBitsPerSample);
	EXPECT_EQ(0,pFormat->cbSize);
...
TEST(WasAPIDeviceLayer,findClosestFormatTypeSupportedFor16Bits48000HzAnd2ChannelsWhen8Bits32000HzAnd8ChannelsSupported)
{
	...
	WAVEFORMATEX formatSupported;
	device.testDefaultWaveFormat(formatSupported);
	device.testSetWaveFormat(8,8,32000,formatSupported);
	...
	EXPECT_EQ(8,pFormat->nChannels);
	EXPECT_EQ(32000,pFormat->nSamplesPerSec);
	EXPECT_EQ(32000 * 1 * 8,pFormat->nAvgBytesPerSec);
	EXPECT_EQ(8,pFormat->nBlockAlign);
	EXPECT_EQ(8,pFormat->wBitsPerSample);
	EXPECT_EQ(0,pFormat->cbSize);
...
TEST(WasAPIDeviceLayer,findClosestFormatTypeSupportedFor32BitFloat48000HzAnd4ChannelsWhen8Bits88200HzAnd3ChannelsSupported)
{
	...
	device.testDefaultWaveFormat(formatActual);
	formatActual.wFormatTag = WasAPIDeviceLayer::e_formatFloat;
	device.testSetWaveFormat(4,32,48000,formatActual);
	
	WAVEFORMATEX formatSupported;
	device.testDefaultWaveFormat(formatSupported);
	device.testSetWaveFormat(3,8,88200,formatSupported);
	...
	EXPECT_EQ(WAVE_FORMAT_PCM,pFormat->wFormatTag);
	EXPECT_EQ(3,pFormat->nChannels);
	EXPECT_EQ(88200,pFormat->nSamplesPerSec);
	EXPECT_EQ(88200 * 1 * 3,pFormat->nAvgBytesPerSec);
	EXPECT_EQ(3,pFormat->nBlockAlign);
	EXPECT_EQ(8,pFormat->wBitsPerSample);
	EXPECT_EQ(0,pFormat->cbSize);
...
TEST(WasAPIDeviceLayer,findClosestFormatTypeSupportedFor16Bits48000HzAnd8ChannelsWhen8Bits32000HzAnd2ChannelsSupported)
{
	...
	WAVEFORMATEX formatSupported;
	device.testDefaultWaveFormat(formatSupported);
	device.testSetWaveFormat(2,8,32000,formatSupported);
	...
	EXPECT_EQ(2,pFormat->nChannels);
	EXPECT_EQ(32000,pFormat->nSamplesPerSec);
	EXPECT_EQ(32000 * 1 * 2,pFormat->nAvgBytesPerSec);
	EXPECT_EQ(2,pFormat->nBlockAlign);
	EXPECT_EQ(8,pFormat->wBitsPerSample);
	EXPECT_EQ(0,pFormat->cbSize);
...
class WasAPIDeviceLayerNameTest : public WasAPIDeviceLayerTest
{
	public:
		WasAPIDeviceLayerNameTest(IMMDeviceIFSPtr pDevice,IAudioClientIFSPtr pAudioClient);
		MOCK_METHOD1(createPropertyStoreIF,IPropertyStoreIFSPtr(IPropertyStore *pPropertyStore));
};

//-------------------------------------------------------------------------------------------

WasAPIDeviceLayerNameTest::WasAPIDeviceLayerNameTest(IMMDeviceIFSPtr pDevice,IAudioClientIFSPtr pAudioClient) : WasAPIDeviceLayerTest(pDevice,pAudioClient)
{}

//-------------------------------------------------------------------------------------------

class WasAPIDeviceLayerNameInvoker
{
	public:
		WasAPIDeviceLayerNameInvoker(const QString& n);
...
WasAPIDeviceLayerNameInvoker::WasAPIDeviceLayerNameInvoker(const QString& n) : m_name(n)
{}

//-------------------------------------------------------------------------------------------

HRESULT WasAPIDeviceLayerNameInvoker::GetValue(REFPROPERTYKEY,PROPVARIANT *pv)
{
	pv->pwszVal = reinterpret_cast<LPWSTR>(CoTaskMemAlloc((m_name.length() + 1) * sizeof(tuint16)));
	memcpy(pv->pwszVal,m_name.utf16(),m_name.length() * sizeof(tuint16));
	pv->pwszVal[m_name.length()] = 0;
	return S_OK;
}

//-------------------------------------------------------------------------------------------

TEST(WasAPIDeviceLayer,nameFailsToGetPropertyStore)
{
	IPropertyStore *iProperties = reinterpret_cast<IPropertyStore *>(6);
	
	IMMDeviceIFSPtr pDevice(new IMMDeviceIFMock());
	IMMDeviceIFMock& mockDevice = dynamic_cast<IMMDeviceIFMock&>(*(pDevice.data()));
	IAudioClientIFSPtr pAudioClient(new IAudioClientIFMock());
	EXPECT_CALL(mockDevice,OpenPropertyStore(STGM_READ,A<IPropertyStore **>())).Times(1)
		.WillOnce(DoAll(SetArgPointee<1>(iProperties),Return(S_FALSE)));

	WasAPIDeviceLayerNameTest device(pDevice,pAudioClient);
	
	ASSERT_TRUE(device.name().isEmpty());	
}

//-------------------------------------------------------------------------------------------

TEST(WasAPIDeviceLayer,nameNoPropertyStoreGiven)
{
	...
	IAudioClientIFSPtr pAudioClient(new IAudioClientIFMock());
	EXPECT_CALL(mockDevice,OpenPropertyStore(STGM_READ,A<IPropertyStore **>())).Times(1)
		.WillOnce(DoAll(SetArgPointee<1>(iProperties),Return(S_OK)));
...
TEST(WasAPIDeviceLayer,nameFailToGetFriendlyName)
{
	IPropertyStore *iProperties = reinterpret_cast<IPropertyStore *>(6);
	IPropertyStoreIFSPtr pPropertyStore(new IPropertyStoreIFMock());
	IPropertyStoreIFMock& mockPropertyStore = dynamic_cast<IPropertyStoreIFMock&>(*(pPropertyStore.data()));
	EXPECT_CALL(mockPropertyStore,GetValue(A<REFPROPERTYKEY>(),A<PROPVARIANT *>())).Times(1).WillOnce(Return(S_FALSE));
	...
	WasAPIDeviceLayerNameTest device(pDevice,pAudioClient);
	EXPECT_CALL(device,createPropertyStoreIF(iProperties)).Times(1).WillOnce(Return(pPropertyStore));
...
TEST(WasAPIDeviceLayer,nameSuccess)
{
	QString deviceName = "SoundCard";
	WasAPIDeviceLayerNameInvoker invoker(deviceName);
	
	IPropertyStore *iProperties = reinterpret_cast<IPropertyStore *>(6);
	IPropertyStoreIFSPtr pPropertyStore(new IPropertyStoreIFMock());
	IPropertyStoreIFMock& mockPropertyStore = dynamic_cast<IPropertyStoreIFMock&>(*(pPropertyStore.data()));
	EXPECT_CALL(mockPropertyStore,GetValue(A<REFPROPERTYKEY>(),A<PROPVARIANT *>())).Times(1)
		.WillOnce(Invoke(&invoker,&WasAPIDeviceLayerNameInvoker::GetValue));
...
TEST(WasAPIDeviceLayer,saveAndLoadFormats)
{
	...
	QString deviceName = "Audio Codec Device ID #5612";
	WasAPIDeviceLayerSaveAndLoadFormatsInvoker invoker(deviceName);

	IMMDeviceIFSPtr pDevice(new IMMDeviceIFMock());
	IMMDeviceIFMock& mockDevice = dynamic_cast<IMMDeviceIFMock&>(*(pDevice.data()));
	IAudioClientIFSPtr pAudioClient(new IAudioClientIFMock());

	EXPECT_CALL(mockDevice,GetId(A<LPWSTR *>())).WillRepeatedly(Invoke(&invoker,&WasAPIDeviceLayerSaveAndLoadFormatsInvoker::GetId));

	WasAPIDeviceLayerTest device(pDevice,pAudioClient);

	tint i,j,k;
	tint m_expectedFormatExclusive[NUMBER_WASAPI_MAXCHANNELS][NUMBER_WASAPI_MAXBITS][NUMBER_WASAPI_MAXFREQUENCIES];
	tint m_expectedFormatShared[NUMBER_WASAPI_MAXCHANNELS][NUMBER_WASAPI_MAXBITS][NUMBER_WASAPI_MAXFREQUENCIES];
	...
	for(i=0;i<NUMBER_WASAPI_MAXCHANNELS;i++)
	{
		for(j=0;j<NUMBER_WASAPI_MAXBITS;j++)
		{
			...
			{
				tint v = static_cast<tint>(common::Random::instance()->randomUInt32() & 0x0000007f);
				if(common::Random::instance()->randomUInt32() & 0x00000001)
				{
					v = -v;
				}
				device.setFormatExclusive(i,j,k,v);
				m_expectedFormatExclusive[i][j][k] = v;
				
				v = static_cast<tint>(common::Random::instance()->randomUInt32() & 0x0000007f);
				if(common::Random::instance()->randomUInt32() & 0x00000001)
				{
					v = -v;
				}
				device.setFormatShared(i,j,k,v);
				m_expectedFormatShared[i][j][k] = v;
			}
		}
	}
	
	ASSERT_TRUE(device.testSaveFormats());
	...
	ASSERT_TRUE(device.testLoadFormats());
...
TEST(WasAPIDeviceLayer,findClosestSupportedFormatWhenClosestNotFound)
{
	...
	EXPECT_CALL(deviceLayer,findClosestWaveFormatFromDescription(sourceDesc)).Times(1).WillOnce(Return(reinterpret_cast<WAVEFORMATEX *>(0)));
...
TEST(WasAPIDeviceLayer,findClosestSupportedFormatWithExclusiveAndNonExIsSupported)
{
	...
	EXPECT_CALL(mockAudio,IsFormatSupported(AUDCLNT_SHAREMODE_EXCLUSIVE,WaveFormatExIsExpected(formatA),0))
...
TEST(WasAPIDeviceLayer,findClosestSupportedFormatWithExclusiveAndNonExIsNotSupportedAndExtIsSupported)
{
	...
	EXPECT_CALL(mockAudio,IsFormatSupported(AUDCLNT_SHAREMODE_EXCLUSIVE,WaveFormatExIsExpected(formatA),0))
		.Times(1).WillOnce(Return(S_FALSE));
	EXPECT_CALL(mockAudio,IsFormatSupported(AUDCLNT_SHAREMODE_EXCLUSIVE,WaveFormatExtensibleIsExpected(formatExtA),0))
		.Times(1).WillOnce(Return(S_OK));
...
TEST(WasAPIDeviceLayer,findClosestSupportedFormatWithSharedAndNonExIsSupported)
{
	...
	formatClosestA = reinterpret_cast<WAVEFORMATEX *>(CoTaskMemAlloc(sizeof(WAVEFORMATEX)));
...
TEST(WasAPIDeviceLayer,findClosestSupportedFormatWithSharedAndNonExIsNotSupportedAndExtIsSupported)
{
	...
	WAVEFORMATEXTENSIBLE *formatClosestExtA = reinterpret_cast<WAVEFORMATEXTENSIBLE *>(CoTaskMemAlloc(sizeof(WAVEFORMATEXTENSIBLE)));
	...
	EXPECT_CALL(mockAudio,IsFormatSupported(AUDCLNT_SHAREMODE_SHARED,WaveFormatExtensibleIsExpected(formatExtA),A<WAVEFORMATEX **>()))
		.Times(1).WillOnce(DoAll(SetArgPointee<2>(reinterpret_cast<WAVEFORMATEX *>(formatClosestExtA)),Return(S_OK)));
...
TEST(WasAPIDeviceLayer,findClosestSupportedFormatWithSharedAndNonExAndExtIsNotSupported)
{
	...
	EXPECT_CALL(mockAudio,IsFormatSupported(AUDCLNT_SHAREMODE_SHARED,WaveFormatExIsExpected(formatA),A<WAVEFORMATEX **>()))
		.Times(1).WillOnce(DoAll(SetArgPointee<2>(formatClosestA),Return(S_FALSE)));
	EXPECT_CALL(mockAudio,IsFormatSupported(AUDCLNT_SHAREMODE_SHARED,WaveFormatExtensibleIsExpected(formatExtA),A<WAVEFORMATEX **>()))
		.Times(1).WillOnce(DoAll(SetArgPointee<2>(reinterpret_cast<WAVEFORMATEX *>(formatClosestExtA)),Return(S_FALSE)));
