...
void Player::onShowLicense()
{
#if !defined(OMEGA_MAC_STORE)
    widget::LicenseRegisterDialog dlg(this);
#if defined(OMEGA_WIN32)
	QObject::connect(&dlg,SIGNAL(onUpdate()),this,SLOT(onUpdateSoftware()));
#endif
	dlg.exec();
#if defined(OMEGA_WIN32)
	QObject::disconnect(&dlg,SIGNAL(onUpdate()),this,SLOT(onUpdateSoftware()));
#endif
#else
    widget::AboutDialog dlg(this);
	dlg.exec();
#endif
}
...
void Player::onLicensed()
{
    ...
    if(lClient.data()!=0)
	{
		...
		{
			int noDays;
			common::TimeStamp endDate = lClient->endOfTrialDate();
			...
			bTxt = "Register Black Omega. You have " + QString::number(noDays) + " day";
			if(noDays>1)
			{
				bTxt += "s";
			}
			bTxt += " left on your trial.";
		}
		else
		{
			bTxt = "Licensed to " + lClient->getCustomerName() + ".";
		}
	}
	ui.m_licenseButton->setToolTip(bTxt);
#endif
}

//-------------------------------------------------------------------------------------------

void Player::onNotLicensed()
{
#if !defined(OMEGA_MAC_STORE)
	QString bTxt = "Unlicensed copy of Black Omega. Please purchase a license.";
	ui.m_licenseButton->setToolTip(bTxt);
#endif
}
